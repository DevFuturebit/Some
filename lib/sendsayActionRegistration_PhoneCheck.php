...
class sendsayActionRegistration_PhoneCheck extends sendsayAction
{
    public $sub_send_params = Array(
        "action" => "issue.send",
        "letter" => array("from.name" => "lensmaster", "draft.id" => "422"),
        "group" => "personal",
        "mute" => "1",
        "sendwhen" => "now");
    ...
    public function subscribeAction($data)
    {
        ...
        $this->sub_subscribe_params['obj']['a848']['q493'] = $data['checkcode'];
        $this->sub_subscribe_params['obj']['a848']['q398'] = date("Y-m-d H:i");
